...
import { searchVideos, getVideoDetails, getSearchSuggestions } from '../api/youtube';
    ...
    try {
      ...
      if (videoIds) {
        try {
          ...
          // If details fetch fails, use search results without stats
          const videosWithoutStats = searchResults.map(item => ({
            id: item.id.videoId,
            snippet: item.snippet,
          }));
